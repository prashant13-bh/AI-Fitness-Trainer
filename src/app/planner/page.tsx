...
export default function PlannerPage() {
  ...
  return (
    <div style={{ background: 'var(--bg-surface)', minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '1.25rem 1rem 0', flexShrink: 0 }}>
        {/* Header */}
        <div style={{ marginBottom: '1rem' }} className="animate-fadeInUp">
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em' }}>AI-Powered</p>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', background: 'rgba(255,255,255,0.04)', borderRadius: '14px', padding: '4px', marginBottom: '1rem' }}>
          {(['plan', 'chat'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{
              flex: 1, padding: '0.6rem', borderRadius: '10px', border: 'none', cursor: 'pointer',
              fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.9rem', transition: 'all 0.3s',
              background: activeTab === tab ? 'linear-gradient(135deg, rgba(0,212,255,0.15), rgba(123,47,190,0.15))' : 'transparent',
              color: activeTab === tab ? 'var(--ice-blue)' : 'var(--text-muted)',
            }}>
              {tab === 'plan' ? '📅 Weekly Plan' : '🤖 AI Coach'}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* PLAN TAB */}
      {activeTab === 'plan' && (
        <div style={{ flex: 1, overflow: 'auto', padding: '0 1rem', paddingBottom: '6rem' }}>
          {/* Generate button */}
          <div style={{ marginBottom: '1rem' }}>
            <button onClick={regeneratePlan} style={{
              width: '100%', padding: '0.875rem', borderRadius: '14px',
              background: generating ? 'rgba(0,212,255,0.05)' : 'linear-gradient(135deg, rgba(0,212,255,0.1), rgba(123,47,190,0.1))',
              border: '1px solid rgba(0,212,255,0.2)', color: 'var(--ice-blue)',
              fontFamily: 'var(--font-display)', fontWeight: 700, cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem',
              transition: 'all 0.3s',
            }}>
              {generating ? '⏳ Generating your personalized plan...' : '🤖 Regenerate Plan with AI'}
            </button>
          </div>
          ...
          <div className="animate-scaleIn">
            {/* Workout Card */}
            <div style={{ marginBottom: '1rem' }}>
              <div className="section-title" style={{ marginBottom: '0.5rem' }}>Workout</div>
              {day.isRestDay || !day.workout ? (
                <div className="glass-card" style={{ padding: '1rem', borderRadius: '16px', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div>
                    <div className="font-display" style={{ fontWeight: 700, color: '#10B981' }}>Rest Day</div>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Light walk or yoga recommended</div>
                  </div>
                </div>
              ) : (
            ...
            {/* Meals */}
            <div className="section-title" style={{ marginBottom: '0.5rem' }}>Meal Plan</div>
            ...
            {/* Day Totals */}
            <div className="glass-card-primary" style={{ padding: '1rem', borderRadius: '16px', display: 'flex', justifyContent: 'space-around' }}>
      ...
      {/* CHAT TAB */}
      {activeTab === 'chat' && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: '0 1rem', paddingBottom: '6rem' }}>
          {/* Messages */}
          <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.875rem', paddingBottom: '1rem' }}>
            {messages.map((msg, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                {msg.role === 'ai' && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', marginBottom: '0.35rem' }}>
                    ...
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>AI Coach</span>
                  </div>
                )}
                <div className={msg.role === 'ai' ? 'chat-bubble-ai' : 'chat-bubble-user'}
                  style={{ whiteSpace: 'pre-wrap', fontSize: '0.88rem', lineHeight: 1.55 }}>
                  {msg.text}
                </div>
              </div>
            ))}
          ...
          {/* Quick Prompts */}
          <div style={{ display: 'flex', gap: '0.5rem', overflowX: 'auto', paddingBottom: '0.5rem', scrollbarWidth: 'none', marginBottom: '0.75rem', flexShrink: 0 }}>
            {['Adjust my plan', 'Best protein sources', 'Recovery tips', 'Motivation'].map(q => (
              <button key={q} onClick={() => setInput(q)}
                style={{ flexShrink: 0, padding: '0.4rem 0.875rem', borderRadius: '20px', border: '1px solid rgba(0,212,255,0.2)', background: 'rgba(0,212,255,0.05)', color: 'var(--ice-blue)', fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-display)' }}>
                {q}
              </button>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexShrink: 0 }}>
            <input className="input-field" style={{ flex: 1, borderRadius: '14px' }} placeholder="Ask your AI coach..." value={input}
              onChange={e => setInput(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && sendMessage()} />
            <button onClick={sendMessage} disabled={!input.trim() || isTyping}
              style={{ width: '48px', height: '48px', borderRadius: '14px', background: input.trim() ? 'linear-gradient(135deg, #00D4FF, #7B2FBE)' : 'rgba(255,255,255,0.05)', border: 'none', cursor: input.trim() ? 'pointer' : 'default', fontSize: '1.1rem', transition: 'all 0.3s', flexShrink: 0 }}>
              ↑
            </button>
          </div>
        </div>
